...
public class MembersController {
	...
	public void initialize() throws IOException {
		...
		maritalStatusCol.setOnEditCommit(event -> {
			...
			if (!addNewMaritalStatus(member.getMaritalStatus())) {
				member.setMaritalStatus(event.getOldValue());
				return;
			}
			database.updateMember(member);
		});
	...
	public void setDatabase(Database database) {
		...
		database.connectedProperty().addListener((observable, oldValue, newValue) -> {
			...
			if (newValue == true) {
				...
				maritalStatusCol.setCellFactory(column -> {

					ComboBoxTableCell<Member, MaritalStatus> cell = new ComboBoxTableCell<Member, MaritalStatus>();

					cell.setConverter(new StringConverter<MaritalStatus>() {
					...
					cell.getItems().addAll(maritalStatus);
	...
	public void addMember() {
		...
		memberController.setEditable(true);
